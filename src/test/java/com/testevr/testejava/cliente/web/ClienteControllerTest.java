...
class ClienteControllerTest {
    @Test
    void testCriarClienteComErro() {
        ClienteDto dto = new ClienteDto(1L, "Nome", "Razao", "Fantasia", "69423022000160", true);
        when(mapper.toEntity(dto)).thenThrow(new RuntimeException("erro"));
        assertThrows(RuntimeException.class, () -> controller.criarCliente(dto));
    }

    @Test
    void testBuscarTodosVazio() {
        when(clienteService.findAll()).thenReturn(java.util.Collections.emptyList());
    ...
    @Test
    void testDeletarMultiplos() {
        doNothing().when(clienteService).delete(1L);
        doNothing().when(clienteService).delete(2L);
        controller.deletar(1L);
        controller.deletar(2L);
        verify(clienteService).delete(1L);
        verify(clienteService).delete(2L);
    }
    @Mock
