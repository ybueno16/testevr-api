...
public class VendaController {
    ...
    public ResponseEntity<VendaDto> processarVenda(@RequestBody VendaDto vendaDto) {
        try {
            ...
            // 3. Preparar request para API externa (se necessário)
            VendaExternaRequest requestExterna = mapper.toExternaRequest(vendaCriada);

            // 4. Chamar API externa (se necessário)
            VendaExternaResponse responseExterna = vendaExternaService.processarVenda(requestExterna);
